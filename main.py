...
from times_train import times_run_training
import os
...
def get_args():
    ...
    parser.add_argument("--cfg", default="TimesUnet.yaml", type=str)
...
if __name__ == "__main__":
    try:
        ...
        anonymous = None
        wandb.login(key="your_keyword") #rmwang
    except:
        anonymous = "must"
        print(
    ...
    for fold in range(1):
        ...
        train_loader, valid_loader, test_loader = prepare_loaders(df_test = df_test, df=df, fold=fold, CFG=CFG,
                                                    debug=CFG['debug'],
        ...
        if CFG['use_timestamp'] == True:

            timestamp_model = DaViT().to(CFG['device'])
            timestamp_model.load_state_dict(torch.load(f"/home8t/rgye/seg_rgye/model_save/timesViT_best.bin"))
            # timestamp_model.eval()

            model, history = times_run_training(timestamp_model, model, optimizer, scheduler,
                                          device=CFG['device'],
