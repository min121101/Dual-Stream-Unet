...
def train_one_epoch(model, optimizer, scheduler, dataloader, device, epoch, lambda_, CFG):
    model.train()
    ...
    gc.collect()
    PATH = f"model_save/epoch_{epoch}_timesViT_loss_{epoch_loss}.bin"
...
@torch.no_grad()
def test_model(model, optimizer, dataloader, device, CFG):
    model.eval()
...
def run_training(model, optimizer, scheduler, num_epochs, CFG, device, train_loader, valid_loader, test_loader,  run, fold):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        val_loss, LB_val_scores, SB_val_scores, ST_val_scores, val_scores = valid_one_epoch(model, optimizer, valid_loader, device, epoch=epoch, CFG=CFG)
    ...
    val_loss, LB_val_scores, SB_val_scores, ST_val_scores, val_scores = test_model(model, optimizer, test_loader, device, CFG = CFG)
    val_dice, val_jaccard = val_scores
    LB_val_dice, LB_val_jaccard = LB_val_scores
    SB_val_dice, SB_val_jaccard = SB_val_scores
    ST_val_dice, ST_val_jaccard = ST_val_scores
    ...
    print("Best Score: {:.4f}".format(val_dice))
