...
class DaViT(nn.Module):
    ...
    def __init__(self, in_chans=3, depths=(1, 1, 3, 1), patch_size=4,
                 embed_dims=(64, 128, 192, 256), num_heads=(4, 8, 12, 16), window_size=7, mlp_ratio=4.,
        ...
        # self.relu = nn.ReLU()
        self.linear1 = nn.Linear(30720, 10000)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            H, W = sizes[i]
            outs = x_out.view(-1, H, W, self.embed_dims[i]).permute(0, 3, 1, 2).contiguous()

        # outs = self.conv1x1(outs)
        # outs = self.relu(outs)
        timestamp = self.linear1(outs.view(outs.shape[0], -1))
        timestamp = self.sigmoid(timestamp)
        timestamp = self.linear2(timestamp)
        # timestamp = self.relu(timestamp)
